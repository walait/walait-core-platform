...
import type { PermissionService } from '../../application/permission.service';
...
describe('PermissionsGuard', () => {
  ...
  beforeEach(() => {
    ...
    permissionService = { listPermissions: vi.fn() } as unknown as PermissionService;
  ...
  it('throws Unauthorized when no user is present', async () => {
    (reflector.getAllAndOverride as unknown as ReturnType<typeof vi.fn>).mockReturnValue(['read']);
  ...
  it('throws Forbidden when user lacks permission', async () => {
    (reflector.getAllAndOverride as unknown as ReturnType<typeof vi.fn>).mockReturnValue(['read']);
    (permissionService.listPermissions as unknown as ReturnType<typeof vi.fn>).mockResolvedValue([
      'write',
    ]);
