...
describe('PasswordController', () => {
  ...
  describe('resetPassword', () => {
    ...
    it('should reset password if token is valid', async () => {
      ...
      eventBus.send.mockReturnValueOnce(of({ token: 'reset-token' })).mockReturnValueOnce(of(true));
      ...
      const result = await controller.resetPassword('valid-token', 'new-password', req);
    ...
    it('should throw NotFoundException if user not found', async () => {
      ...
      await expect(controller.resetPassword('token', 'pass', req)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw NotFoundException if token is invalid', async () => {
      ...
      await expect(controller.resetPassword('invalid-token', 'pass', req)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw NotFoundException if token is expired', async () => {
      userService.findById.mockResolvedValue(user);
      eventBus.send.mockReturnValueOnce(of(null));
      await expect(controller.resetPassword('expired-token', 'pass', req)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
