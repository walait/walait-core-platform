...
export class BaseSoapClient {
  ...
  async call<T>(
    ...
    if (isXml) {
      const soapAction = method === 'loginCms' ? '""' : `http://ar.gov.afip.dif.FEV1/${method}`;
      ...
      try {
        ...
        this.logger.error(`SOAP XML call failed [${method}] → ${err.message}`);
        await writeFile(`${debugPrefix}-error.log`, err.stack || err.toString(), 'utf-8');
        throw err;
      }
    }
    ...
    try {
      ...
      await writeFile(`${debugPrefix}-request.xml`, client.lastRequest, 'utf-8');
      await writeFile(`${debugPrefix}-response.xml`, client.lastResponse, 'utf-8');

      return response as T;
    } catch (err: any) {
      this.logger.error(`SOAP JS call failed [${method}] → ${err.message}`);
      await writeFile(`${debugPrefix}-error.log`, err.stack || err.toString(), 'utf-8');
      throw err;
    }
  }
