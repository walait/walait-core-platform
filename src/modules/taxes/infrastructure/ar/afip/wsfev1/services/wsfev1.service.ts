...
import type { AuthData, FacturacionBuilder, InvoiceRequest } from '../builder/facturacion.builder';
import type { Wsfev1Client } from '../wsfev1.client';
...
export class Wsfev1Service {
  ...
  async getLastInvoice(params: AuthData & { ptoVta: number; cbteTipo: number }) {
    const { path, cleanup } = await this.builder.buildGetUltimoComprobante(params);
    const body = await readFile(path, 'utf8');
    await cleanup();
  ...
  async getInvoiceTypes(auth: AuthData) {
    ...
    const body = await readFile(path, 'utf8');
    await cleanup();
    return this.wsfev1Client.callGetInvoicesTypes(body, this.wsdl, this.endpointUrl);
  }
  ...
  async getCurrencyTypes(auth: AuthData) {
    ...
    const body = await readFile(path, 'utf8');
    await cleanup();
    return this.wsfev1Client.callGetCurrencyTypes(body, this.wsdl, this.endpointUrl);
  }

  async getExchangeRate(auth: AuthData, moendaId: string) {
    const { path, cleanup } = await this.builder.buildGetCotizacion(auth, moendaId);
    const body = await readFile(path, 'utf8');
    await cleanup();
    return this.wsfev1Client.callGetExchangeRate(body, this.wsdl, this.endpointUrl);
  }
  ...
  async getSalesPoint(auth: AuthData) {
    ...
    const body = await readFile(path, 'utf8');
    await cleanup();
    return this.wsfev1Client.callPuntosDeVenta(body, this.wsdl, this.endpointUrl);
  }

  async getCondicionIvaReceptor(params: AuthData & { cbteTipo: number }) {
    const { path, cleanup } = await this.builder.buildGetCondicionIvaReceptor(params);
    const body = await readFile(path, 'utf8');
    await cleanup();
    return this.wsfev1Client.callGetCondicionIvaReceptor(body, this.wsdl, this.endpointUrl);
  }
}
