...
import { BaseSoapClient } from "../shared/services/baseSoapClient.service";
import {
  AuthData,
  FacturacionBuilder,
  InvoiceRequest,
...
export class Wsfev1Client {
  ...
  constructor(
    @Inject(BaseSoapClient)
    private readonly base: BaseSoapClient,
    @Inject(FacturacionBuilder)
  ...
  async callFeCAE(
    ...
    return await this.callClient<InvoiceResponse>(
      body,
      wsdl,
      endpoint,
      "FECAESolicitar",
  ...
  async callClient<T>(
    ...
    this.logger.debug(
      `Calling ${method} on ${endpoint} with WSDL: ${wsdl}`,
      body,
    );
    const debugKey = `${method}`;
    const resp = await this.base.call<T>(
      wsdl,
      endpoint,
      method,
      body,
    ...
    const result =
      parsedBody?.[responseKey]?.[resultKey] ?? parsedBody?.[responseKey]; // fallback si no hay Result explícito

    if (!result) {
      this.logger.error(
        `No se encontró '${resultKey}' en la respuesta de ${method}`,
        parsedBody,
      );
