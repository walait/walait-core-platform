...
export function generateStrongPassphrase(length = 48): string {
  if (length < 12) throw new Error('Passphrase should be at least 12 characters');
  ...
  const remaining = Array.from(randomBytes(length - mandatory.length), (b) => all[b % all.length]);
...
const ENCRYPTION_KEY = scryptSync(process.env.PKI_MASTER_SECRET, 'salt', 32); // 32 bytes
const IV_LENGTH = 12; // Para GCM
...
export function encryptPem(pem: string): string {
  ...
  return Buffer.concat([iv, tag, encrypted]).toString('base64'); // ⇢ guarda esto en la BD
}
