...
describe("VerificationController", () => {
  ...
  describe("verifyEmail", () => {
    ...
    it("verifies email and returns redirect data", async () => {
      verificationService.getEmailVerificationToken.mockResolvedValue({
        token: tokenPayload,
      });
      ...
      expect(eventBus.send).toHaveBeenNthCalledWith(
        3,
    ...
    it("throws BadRequest when token is missing", async () => {
      ...
      await expect(
        controller.verifyEmail("invalid", "org-123"),
    ...
    it("throws BadRequest when email is already verified", async () => {
      ...
      eventBus.send.mockReturnValueOnce(
        of({ id: "user-1", is_email_verified: true }),
      );
  ...
  describe("resendVerification", () => {
    ...
    it("resends verification and returns token data", async () => {
      ...
      eventBus.send
        ...
        .mockReturnValueOnce(of("jwt-token"));
      ...
      const result = await controller.resendVerification(
        reqFactory({ sub: existingUser.id }),
      );
    ...
    it("throws BadRequest when req.user is missing", async () => {
      await expect(
        controller.resendVerification(reqFactory(null)),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("throws BadRequest when user is not found", async () => {
      eventBus.send.mockReturnValueOnce(of(null));
      await expect(
        controller.resendVerification(reqFactory({ sub: "ghost" })),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("throws BadRequest when email already verified", async () => {
      ...
      await expect(
        controller.resendVerification(reqFactory({ sub: "user-1" })),
      ).rejects.toThrow(BadRequestException);
    });
  });
});
