...
describe('EmailVerificationService', () => {
  ...
  describe('createToken', () => {
    it('should create and save a new token', async () => {
      ...
      const result = await service.createToken(mockUser.id, 'abc123', mockToken.expires_at);
  ...
  describe('getEmailVerificationToken', () => {
    ...
    it('should throw NotFoundException if token not found', async () => {
      ...
      await expect(service.getEmailVerificationToken('invalid')).rejects.toThrow(NotFoundException);
    });

    it('should delete and throw if token is expired', async () => {
      const expiredToken = { ...mockToken, expires_at: new Date(Date.now() - 1000) };
      repo.findOne.mockResolvedValue(expiredToken);

      await expect(service.getEmailVerificationToken('expired')).rejects.toThrow(NotFoundException);
  ...
  describe('createVerificationTokenEmail', () => {
    it('should call createToken with user data', async () => {
      ...
      const result = await service.createToken(mockUser.id, 'abc123', mockToken.expires_at);
      expect(spy).toHaveBeenCalledWith(mockUser.id, 'abc123', mockToken.expires_at);
      expect(result).toEqual(mockToken);
    });
  });
});
