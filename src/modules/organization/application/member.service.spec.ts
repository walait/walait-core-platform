...
const createMockRepo = <T = unknown>(): Partial<
...
describe("MemberService", () => {
  ...
  describe("updateMembership", () => {
    it("updates membership and returns updated entity", async () => {
      ...
      (repo.findOne as Mock).mockResolvedValue(mockMembership);
      const result = await service.updateMembership("mem-1", {
        user_id: "asdf-123",
      });
  ...
  describe("getMembershipAndOrg", () => {
    ...
    it("throws NotFoundException if not found", async () => {
      ...
      await expect(service.getMembershipAndOrg("user-1")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
