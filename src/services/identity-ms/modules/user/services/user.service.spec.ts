...
describe('UserService', () => {
  ...
  it('should map user to response object', () => {
    ...
    expect(result).toEqual(expect.objectContaining({ id: '1', email: 'email@test.com' }));
  });
  ...
  it('should create user and hash password', async () => {
    ...
    (repo.create as Mock).mockReturnValue({ ...input, password_hash: 'hashed' });
    (repo.save as Mock).mockResolvedValue({ id: '1', ...input, password_hash: 'hashed' });

    const result = await service.createUser(input);

    expect(repo.create).toHaveBeenCalledWith(expect.objectContaining({ password_hash: 'hashed' }));
