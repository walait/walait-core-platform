...
export class AdminService {
  ...
  async getAllUser(reqUserRole: string, reqOrganizationId: string) {
    ...
    for (const user of users) {
      ...
      const userRole = userRolesByUser.find((role) => role.organization_id === reqOrganizationId);
    ...
    return mapped.filter((user) => {
      ...
      return currentHierarchy >= userHierarchy && user.organization_id === reqOrganizationId;
    });
  }
}
