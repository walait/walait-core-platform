import type { RoleType } from '@/services/access-ms/modules/role/types/role.type';
...
describe('RoleService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [RoleService, { provide: getRepositoryToken(Role), useValue: repo }],
    }).compile();
  ...
  it('getRoleByNameAndScope busca por name y scope', async () => {
    ...
    const res = await service.getRoleByNameAndScope('admin' as RoleType, 'organization');
  ...
  it('updateRole actualiza valores y guarda', async () => {
    ...
    repo.save.mockResolvedValue({ ...existing, name: 'owner', organization_id: 'org-2' });
